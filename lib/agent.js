...
export class OllamaCodeAgent {
    ...
    async askAi(cwd) {
        const systemMsg = `You are 'OllamaCode', an autonomous terminal agent. Context: ${getSystemContext(cwd)} Suggest bash commands in \`\`\`bash ... \`\`\` blocks. If the task is complete, STOP.`;
        this.clipHistory();
