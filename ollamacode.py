...
class OllamaCodeAgent:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for m in models_data:
                    ...
                    refined_models.append({"id": m['name'], "family": details.get('family', 'N/A')})
        ...
        while step <= 5:
            ...
            if step == 1:
                ...
                if self.provider == "Groq":
                    self.api_key = Prompt.ask("Groq Key", password=True, default=self.api_key); step = 4
                else:
                    self.ollama_url = Prompt.ask("Ollama URL", default=self.ollama_url); step = 3
            elif step == 3:
                ...
                if not models: self.model = Prompt.ask("Model Name", default=self.model); step = 4
                    ...
                    self.model = models[int(m_choice)-1]['id']; step = 4
            elif step == 4:
                if self.provider == "Groq":
    ...
    def ask_ai(self):
        system_msg = ("You are 'OllamaCode', an autonomous terminal agent. Context: " + self.get_system_context() + " Suggest bash commands in ```bash ... ``` blocks. IMPORTANT: If output shows task is complete, STOP.")
        messages = [{"role": "system", "content": system_msg}] + self.history[-15:]
        try:
            if self.provider == "Groq":
                response = requests.post(self.groq_url, headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}, json={"messages": messages, "model": self.model}, timeout=30)
    ...
    def execute_command(self, cmd):
        ...
        try:
            if cmd.strip().startswith("cd "):
                ...
                os.chdir(os.path.expanduser(new_dir)); self.cwd = os.getcwd(); return 0, f"Changed to {self.cwd}"
            ...
            while True:
                ...
                while True:
                    with Progress(SpinnerColumn(), TextColumn(f"[cyan]{self.model}...[/cyan]"), transient=True) as progress:
                        progress.add_task("", total=None); answer = self.ask_ai()
                    ...
                    for cmd in commands:
                        ...
                        if self.auto_run or Confirm.ask("Execute?"):
                            ...
                            if (cmd, stdout) in recent_actions: self.console.print("[bold red]Loop detected![/]"); break
                            ...
                            if status != 0 and self.auto_fix:
                                self.history.append({"role": "user", "content": "Previous command failed. Fix it."}); break
                        else: break
...
def main_update():
    ...
    if not os.path.exists(os.path.join(repo_path, ".git")): print("Not a git repository."); return
    try:
        ...
        if local_hash == remote_hash:
            print("[bold green]OllamaCode is already up to date.[/bold green]")
            return
        print("[yellow]Updating OllamaCode...[/yellow]")
        subprocess.run(["git", "pull", "origin", "main"], cwd=repo_path)
        subprocess.run([sys.executable, "-m", "pip", "install", "-e", "."], cwd=repo_path)
        print("[green]Update successful! Please restart.[/green]")
    except Exception as e: print(f"Update failed: {e}")

def main():
    if len(sys.argv) > 1:
        if sys.argv[1] == "update": main_update(); return
        if sys.argv[1] == "settings": OllamaCodeAgent().run(force_setup=True); return
